...
public class LojaServiceImp implements LojaService{
    ...
    @Override
    public Optional<LojaDto> obterEstacionadoPorId(String id) {
        
        Optional<Loja> estacionado = repository.findById(id);
        if(estacionado.isPresent()){
            LojaDto estacionadoComCarro = new ModelMapper().map(estacionado, LojaDto.class);
    ...
    public Optional<LojaDto> atualizarCarroPorId(String id, LojaDto dto) {
        ...
        if(retorno.isPresent()){
            ...
            return Optional.of(new ModelMapper().map(estacionadoRetorno, LojaDto.class));
        }else{
            return Optional.empty();
        }
    }   
