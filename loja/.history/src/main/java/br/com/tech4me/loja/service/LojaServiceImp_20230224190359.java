...
public class LojaServiceImp implements LojaService{
    ...
    public LojaCompletoDto CadastrarCompra(LojaCompletoDto dto) {
        String idSkin = dto.getIdSkin();
        Optional<Loja> lojaExistente = repository.findById(idSkin);
        
        if(lojaExistente.isPresent()) {
            throw new SkinJaCadastradaException("A skin já está cadastrada no sistema.");
        }
        
        Loja comprado = modelMapper.map(dto, Loja.class);
        compraClient.atualizarVendido(idSkin, true);
        repository.save(comprado);
        return modelMapper.map(comprado, LojaCompletoDto.class);
    }

        
        // atualizar o produto como vendido
