...
import br.com.tech4carro.carro.service.ProdutoService;
...
public class ProdutoController {
  ...
  @PostMapping
  public ResponseEntity<ProdutoCompletoDto> cadastrarCarro(@RequestBody @Valid ProdutoCompletoDto Carro){
    return new ResponseEntity<>(servico.cadastrar(Carro), HttpStatus.CREATED); 
  }

  @GetMapping
  public ResponseEntity<List<ProdutoCompletoDto>> obterEstoque() {
    return new ResponseEntity<>(servico.obterTodos(), HttpStatus.OK);
  }

  @GetMapping("/{id}")
  public ResponseEntity<ProdutoDto> obterCarro(@PathVariable String id) {
  ...
  @PutMapping("/{id}")
  public ResponseEntity<ProdutoCompletoDto> atualizarCarro(@PathVariable String id, @RequestBody @Valid ProdutoCompletoDto Carro){
    Optional<ProdutoCompletoDto> retorno = servico.atualizarPorId(id, Carro); 
